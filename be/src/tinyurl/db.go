...
func NewDB(dbname, user, pass, address, port string) *DBService {
	...
	if err != nil {
		log.Fatal("setup up db error:", err)
	}
...
func (dbs *DBService) Setup() {
	...
	onceSetupDB.Do(func() {
		...
		if err != nil {
			log.Fatal("setup database ", dbs.DBName, " err:", err)
		}
		ret, err := dbs.DB.Exec(urlTable)
		if err != nil {
			log.Fatal("setup table error:", err)
		}

		fmt.Println("setup db success with result:", ret)
	})
}
...
func (dbs *DBService) CheckLongurl(longurl string) (string, bool) {
	...
	defer stmt.Close()
	if err != nil {
		log.Fatal("prepare longurl stmt error: ", err)
	}
...
func (dbs *DBService) InsertShortpath(longurl, shortpath string) {
	...
	defer stmt.Close()
	if err != nil {
		log.Fatal("Insert into database error: ", err)
	}
